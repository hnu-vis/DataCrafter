...
def get_encoder(encoder_name, input_size, input_dims, input_channels):
    try:
        ...
        elif encoder_name == "M2M":
            encoder = FCEncoder(input_dims)
            encoder_out_dims = encoder.hidden_dims[-1]
        elif encoder_name == "MFM":
            encoder = MFMEncoder(input_dims)
            encoder_out_dims = encoder.hidden_dims[-1]
        return encoder, encoder_out_dims
    except:
        raise Exception(
            "Invalid model name. Check the config file and pass one of: resnet18 or resnet50 or CBR or "
            "FC"
        )
...
class FCEncoder(nn.Module):
    def __init__(self, in_features, hidden_dims=None):
        ...
        for dim in hidden_dims:
            modules.append(
                nn.Sequential(nn.Linear(in_dim, dim), nn.BatchNorm1d(dim), nn.ReLU())
            )
            in_dim = dim
        self.encoder = nn.Sequential(*modules)
